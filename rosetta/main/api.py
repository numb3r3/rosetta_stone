...
def xla_train(args, unused_argv):
    """A simple launcher script for TPU training.

    Inspired by https://github.com/huggingface/transformers/blob/master/examples/xla_spawn.py
    """

    if args.num_cores > 1:
        import torch_xla.distributed.xla_multiprocessing as xmp

        xmp.spawn(_xla_train, args=(args, unused_argv), nprocs=args.num_cores)
    else:
        _xla_train(args, unused_argv)


def _xla_train(args, unused_argv):
    from coolname import generate_slug

    # from ..core import trainers
    from ..core.xla import xla_trainer
    from ..helper import create_dataio, create_model, load_yaml_params
    from ..utils.distribute import (
        get_global_rank,
        get_world_size,
    ...
    trainer = xla_trainer.XLATrainer(
    ...
    for epoch in range(hparams["num_epochs"]):
        ...
        trainer.train(para_train_loader, **hparams)

        # evaluate on validation set
        para_eval_loader = pl.ParallelLoader(eval_loader, [self.device])
        eval_metrics = trainer.eval(para_eval_loader, **hparams)
