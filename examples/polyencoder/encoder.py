...
class PolyDSSMBert(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.poly_code_embeddings = nn.Embedding(self.poly_m + 1,
                                                 kwargs['bert_hidden_size'])

        self.dropout = nn.Dropout(kwargs['dropout_rate'])
        self.context_fc = nn.Linear(kwargs['bert_hidden_size'],
                                    kwargs['fc_hidden_size'])
        self.response_fc = nn.Linear(kwargs['bert_hidden_size'],
                                     kwargs['fc_hidden_size'])
...
class DSSMBert(nn.Module):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            dot_product = torch.matmul(context_vec,
                                       responses_vec.permute(
                                           0, 2, 1))  # take this as logits
