...
def main(args):
    ...
    first_hparams = helper.parse_args('app.yaml', args.first_model_name,
                                      'default')
    ...
    first_tokenizer = BertTokenizer(
        vocab_file=first_hparams['tokenizer_name_or_path'])

    second_hparams = helper.parse_args('app.yaml', args.second_model_name,
                                       'default')
    ...
    while True:
        try:
            ...
            with torch.no_grad():
                ...
                print(
                    colored('chatbot #1: ' + ''.join(response_text.split(' ')),
                            'green'))
                ...
                response_text = second_tokenizer.decode(
                    response_ids, skip_special_tokens=True)
                print(
                    colored('chatbot #2: ' + ''.join(response_text.split(' ')),
                            'cyan'))
                history.append(response_ids)
...
def parse_args():
    ...
    parser.add_argument(
        'first_model_name', type=str, help='the second model name')
    parser.add_argument(
        'second_model_name', type=str, help='the second model name')
