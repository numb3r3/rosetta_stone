...
def LinearAnnealingWithWarmup(warmup_steps,
                              constant_steps,
                              total_training_steps,
                              last_epoch=-1):
    """Set the learning rate of each parameter group using a linear annealing
    schedule."""
    ...
    def lr_lambda(current_step):
        if current_step < warmup_steps:
            ...
            return max(
                0.0,
                total_training_steps - current_step - constant_steps) / max(
                    1.0, total_training_steps - warmup_steps - constant_steps)

    return partial(_lr_scheduler.LambdaLR, **locals())


def CosineAnealingWithWarmup(warmup_steps,
                             constant_steps,
                             total_training_steps,
                             num_cycles=0.5,
                             last_epoch=-1):
    ...
    def get_lr(self):
        ...
        def _lr_lambda(current_step):
            ...
            if current_step < self.warmup_steps:
                warmup_percent_done = current_step / max(
