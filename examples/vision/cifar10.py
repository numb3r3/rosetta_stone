...
class CIFAR10(BaseDataIO):
    ...
    def create_dataset(self,
        ...
        assert download or os.path.exists(
            self.cache_path), 'cache path does not exist'

        is_train = mode == 'train'
        transform = (
            transforms.Compose([self.da_transform, self.norm])
            if is_train else self.norm)
    ...
    def collate_fn(self,
        ...
        return (torch.stack(images,
                            dim=0), torch.tensor(labels, dtype=torch.int64))
